...
namespace Simple3D {
	DepthBuffer::DepthBuffer(Device* device, SwapChain* swapchain, VkCommandPool* commandPool) : device(device) {
		VkFormat depthFormat = findDepthFormat(device);

		createImage(swapchain->GetSwapChainExtent().width, swapchain->GetSwapChainExtent().height, depthFormat, VK_IMAGE_TILING_OPTIMAL, VK_IMAGE_USAGE_DEPTH_STENCIL_ATTACHMENT_BIT, VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT, depthImage, depthImageMemory, device);
